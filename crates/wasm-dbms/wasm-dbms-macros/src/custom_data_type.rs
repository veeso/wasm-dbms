...
pub fn custom_data_type(input: &DeriveInput) -> Result<TokenStream> {
    ...
    Ok(quote! {
        impl #impl_generics ::wasm_dbms_api::prelude::CustomDataType for #name #ty_generics #where_clause {
            const TYPE_TAG: &'static str = #type_tag_str;
        }

        impl #impl_generics ::std::convert::From<#name #ty_generics> for ::wasm_dbms_api::prelude::Value #where_clause {
            fn from(val: #name #ty_generics) -> ::wasm_dbms_api::prelude::Value {
                ::wasm_dbms_api::prelude::Value::Custom(::wasm_dbms_api::prelude::CustomValue::new(&val))
            }
        }
    })
}
